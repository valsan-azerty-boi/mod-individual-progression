...
public:
    ...
    bool OnPlayerBeforeTeleport(Player* player, uint32 mapid, float x, float y, float z, float /*orientation*/, uint32 /*options*/, Unit* /*target*/) override
    {
        ...
        if (instanceTemplate)
        {
            ...
            if (instanceTemplate->Parent == MAP_NORTHREND && mapid == MAP_NAXXRAMAS && player->GetLevel() <= IP_LEVEL_TBC && (!sIndividualProgression->isAttuned(player) ||  sIndividualProgression->hasPassedProgression(player, PROGRESSION_TBC_TIER_5) ))
                return false;
        }
    ...
    bool OnPlayerCanGroupAccept(Player* player, Group* group) override
    {
        ...
        if (!sIndividualProgression->isNormalAccount(player)) // player is either a RNDbot or has an excluded account
        {
            ...
            {
                if (groupLeaderState < PROGRESSION_NAXX40) // Group leader is in Vanilla
                {
                ...
                else if (groupLeaderState >= PROGRESSION_NAXX40 && groupLeaderState < PROGRESSION_TBC_TIER_5) // Group leader is in TBC
                {
